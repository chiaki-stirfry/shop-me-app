...
public class CountryRestControllerTests {
	...
	@Test
	@WithMockUser(username = "nam@codejava.net", password = "something", roles = "ADMIN") // username and password can be arbitrary value
	...
	public void testCreateCountry() throws JsonProcessingException, Exception {
		...
		MvcResult result = mockMvc.perform(post(url).contentType("application/json")
				...
				.with(csrf()))  //we must send t he csrf token to the server
			.andDo(print())
			.andExpect(status().isOk())
			.andReturn();
	...
	public void testUpdateCountry() throws JsonProcessingException, Exception {
		...
		mockMvc.perform(post(url).contentType("application/json")
				.content(objectMapper.writeValueAsString(country)) // convert Java object to Json String
				.with(csrf()))
			.andDo(print())
			.andExpect(status().isOk())
